<template>
  <el-form-item
    v-if="element.key && middleWareJudgeType('isShow', element.option)"
  ...
  >
    <!-- input -->
    <template v-if="['text', 'textarea', 'password'].indexOf(element.type) > -1">
      <el-input
    ...
    <template v-if="element.type === 'select'">
      <el-select
        ...
        @change="handleChange"
      >
        <el-option v-for="item in element.option.options" :key="item.value" :label="item.label" :value="item.value" :disabled="item.disabled"> </el-option>
      </el-select>
    </template>
  </el-form-item>
</template>
...
export default {
  ...
  data() {
    const modelForm = JSON.parse(JSON.stringify(this.bindingForm));
    const elementConfig = Object.keys(this.elementInfo).reduce((result, key) => {
      if (['type', 'key', 'prop'].indexOf(key) > -1 && this.elementInfo[key]) {
  ...
  methods: {
    ...
    middleWareJudgeType(key, element, ...args) {
      if (!key || !element) {
        ...
        console.error('meddleWareJudgeType error', key, element);
        return false;
      }
