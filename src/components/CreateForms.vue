<template>
  ...
  >
    <create-form-item
      v-for="elementInfo in elementInfos"
      :key="elementInfo.key"
      :element-info="elementInfo"
      :handle-instance="handleChange"
      :binding-form="createForm"
    />
  </el-form>
</template>
...
export default {
  ...
  computed: {
    rules() {
      const rules = this.elementInfos.reduce(
        (result, elementInfo) => {
          if (elementInfo.option.rule) {
            result[elementInfo.key] = [].concat(
              elementInfo.option.rule
            )
          }
          return result
        },
  ...
  data() {
    ...
    const createForm = elementInfos.reduce(
      (result, element) => {
